...
pix="1.1"
dose="7.05"
patch="5 5" # this does not work!!! change the line 25 yourself
kv="300"
group="1"
frames="7"
#----------------------
...
for i in `ls raw_stacks/ | sed 's/raw_stacks\///g;s/_frames.mrc//g'`;
do
        echo -ne "Aligning stack $k/$total..\r"
        $program -InMrc "raw_stacks/${i}_frames.mrc" -OutMrc "aligned_sums_motioncor2/${i}_aligned_sum.mrc" -Patch 5 5 -FmDose "${dose}" -LogFile "logs/motioncor2/${i}_" -PixSize "${pix}" -kV $kv -Group $group >> logs/motioncor2.log 2>&1
